...
import { saveMark, getAllMarks, UpdateMark, getMarkByID, insertAndCheck } from './mongo.js';
...
// pdf files
let pdfFilesName: string[];
pdfFilesName = fs.readdirSync('./marks/');

console.debug(pdfFilesName);


async function pdf2json(fileName: string): Promise<void> {
...
export interface Student {
    ...
    theoreticalMark: number;
    examMark: number;
}


let jsonFilesName: string[];
...
async function insertData(files: Student[], name: string) {

    files.forEach(async (student: Student) => {
        let std: any = {
...
async function inseForAllMarks(filesName: string[]) {
